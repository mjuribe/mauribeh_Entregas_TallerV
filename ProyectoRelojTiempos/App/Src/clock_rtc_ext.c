/**
 * **************************************************************************************************
 * @file     : clock_rtc_ext.c
 * @author   : Maria Jose Uribe Henao
 * @brief    : WORDCLOCK
 *
 * Este proyecto se basa en un reloj de palabras controlado por un Real Time Clock Externo
 * Cada cinco minutos se presenta la nueva hora en pantalla y por medio de USART se puede configurar
 * la nueva hora del reloj o ver la hora actual del RTC.
 * El encoder permite cambiar de color por medio del giro y por medio del boton refresca el tablero.
 * **************************************************************************************************
 */
...
int main(void) {
	...
	while (1) {
		// Bandera que llama la actualizacion del tablero de letras
		if(tomadedatos==1){
			tomadedatos=0;
			minutero();
		}
...
void initSystem(void) {
	...
	// Se hace una calibracion de la frecuencia del micro
	...
	/*Configuracion del Pin para el control del voltaje de los leds - TRANSISTOR */
	...
	handlerPinC4.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_PUPDR_PULLUP;     // Resistencia Pull up
	...
	handlerPinA11.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_PUPDR_PULLUP;     // Resistencia Pull up
	...
	handlerPinB12.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_PUPDR_PULLUP;     // Resistencia Pull up
...
void callback_extInt4(void){
	if (counterExtiSwitch){
		counterExtiSwitch=0;
		turnofflines();   // Se hace un refresco del tablero de colores
	} else {
		counterExtiSwitch=1;
		tomadedatos=1;
	}
...
void callback_extInt11(void){
	...
	if(counterExtiGiro){
		colorled++; // Ciclo descendente de colores
	}
	else{
		colorled--; // Ciclo ascendente de colores
	}
	if(colorled>5){+
		colorled=0;
	} else if(colorled<1){
	...
	tomadedatos=1; // Bandera para actualizar
}
...
void BasicTimer3_Callback(void) {
	...
	counter++;
	if(tim==10){
		tomadedatos=1; //Actualizacion del tablero de letras
		tim=0;
		colorled++;
		if(colorled>5){
			colorled=0;
...
/* Funcion principal: se lee la hora del RTC
 * Se actualiza los minutos y luego la hora
 * */
void minutero(void){
	min  = RTC_readByte(&handlerRTC,0x01);
	hora = RTC_readByte(&handlerRTC,0x02);
	if(min<5){
		turnoff(&handlerPinTrans);
		delay_ms(100);
		itis(colorled);
		delay_ms(100);
...
/* Funcion secundaria para la hora
 * */
void manecillahora(uint16_t horaconf){
	switch (horaconf){
	case 1:
