...
void parseCommands(char *ptrBufferReception) {
	...
	if (strcmp(cmd, "help") == 0) {
		...
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "4)  mcopres  --> Escribir un unico valor entre 0,2,3,4 o 5 \n");
		writeMsg(&handlerCommTerminal, "    Warning !!! \n");
		writeMsg(&handlerCommTerminal, "    El PLL como minimo debe tener un prescaler de 2 para poder visualizarlo en el osciloscopio \n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "5)  rtcdate --> Configurar la fecha #1 #2 #3 \n");
		...
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "6)  rtctime  --> Configurar la hora  #1 #2 #3 \n");
		...
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "6)  rtcweek  --> Configurar el dia de la semana entre 1-7  \n");
		writeMsg(&handlerCommTerminal, "    A los que corresponden: Lunes(1), Martes(2), Miercoles(3), \n");
		writeMsg(&handlerCommTerminal,	"   Jueves(4), Viernes(5), Sabado(6), Domingo(7)  \n");

		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "7)  actualdate --> Fecha configurada\n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "8)  actualtime --> Hora configurada \n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "9)  adcvel     --> Frecuencia de muestreo en KHz \n");
		writeMsg(&handlerCommTerminal, "    Debe ser un valor entre 10 y 30 \n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "10) adcarray  --> Presentacion de los arreglos de dos señales ADC \n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "11) acceldata  --> Captura de datos \n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "12) accelfreq  --> Frecuencias \n");
		writeMsg(&handlerCommTerminal, "\n");
		writeMsg(&handlerCommTerminal, "NOTAS IMPORTANTES:\n");
		writeMsg(&handlerCommTerminal, "-> Los valores por defecto de los tres parametros son 1000 \n");
		writeMsg(&handlerCommTerminal, "-> Si se desea hacer una actualizacion se debe escribir explicitamente el numero que se desea \n");
		writeMsg(&handlerCommTerminal, "-> El cable blanco en el pin A8 es para medir el MCO1 \n");
		writeMsg(&handlerCommTerminal, "-> Los cables azul (A1) y morado (A0) son del ADC, conectados a GND y 3.3 V en la board \n");
		...
		if (firstParameter == 1 || firstParameter == 2 || firstParameter == 3
			...
			writeMsg(&handlerCommTerminal, bufferData);
			writeMsg(&handlerCommTerminal, "Este es un numero o caracter no especificado\n"
							"Solo son validos los numeros 1, 2, 3, 4 o 5 \n");
		}
		limpiarParam();
		...
		limpiarParam();
	} else if (strcmp(cmd, "rtcweek") == 0) {
		writeMsg(&handlerCommTerminal, "CMD: rtcweek = #Dia de la semana (0-7) \n");
		if (firstParameter > 0 && firstParameter < 8 ) {
			handlerRTC.weekDay = firstParameter;
			config_RTC(&handlerRTC);
			semana(firstParameter);
			writeMsg(&handlerCommTerminal, "El dia de la semana es ");
			writeMsg(&handlerCommTerminal, dia);
			writeMsg(&handlerCommTerminal, "\n");
		} else {
			writeMsg(&handlerCommTerminal, "Dia de la semana no valido\n");
		}
		limpiarParam();
		...
		writeMsg(&handlerCommTerminal, "CMD: adcvel = Frecuencia en KHz \n");
		if(firstParameter>=10 && firstParameter<=30){
			sprintf(bufferData, "Frecuencia de la señal PWM = %u KHz \n", firstParameter);
			writeMsg(&handlerCommTerminal, bufferData);
			handlerSignalPwm1.config.periodo = (1/firstParameter)*1000;
			handlerSignalPwm1.config.duttyCicle = (1/firstParameter)*500;
			pwm_Config(&handlerSignalPwm1);
		} else {
			writeMsg(&handlerCommTerminal, "Frecuencia no valida\n"
					"Debe ser un valor entre 10 y 30\n");
		}
		limpiarParam();
		...
		if (statusInitFFT == ARM_MATH_SUCCESS) {
			arm_rfft_fast_f32(&config_Rfft_fast_f32, datosAccel[2], transformedSignal, ifftFlag);
...
/* Funcion para el dia de la semana */
void semana(uint8_t diaSemana){
	if (diaSemana == 1) {
		sprintf(dia, "Lunes");
	} else if (diaSemana == 2) {
		sprintf(dia, "Martes");
